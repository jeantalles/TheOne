...
const SERVICES = [
  { id: 'estrategia', label: 'Estratégia de Marca',   price: 4000, prazo: '4 semanas' },
  ...
  { id: 'identidade_completa', label: 'Identidade de Marca Completa', price: 6000, prazo: '6 semanas' },
...
// 0: Capa | 1: Dores | 2: SobreTheOne | 3: Jean | 4: Zenic | 5: Thunders | 6: Camilla | 7: TheOne Foundation | 8: Casa da Marca | 9: Estratégia | 10: Naming | 11: Identidade Essencial | 12: Identidade Completa | 13: myBranding | 14: SiteBrandExperience | 15: TheOne Agent | 16: Cronograma | 17: Calculadora | 18: Consultoria
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 1, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', marginTop: '-100px' }}>
        ...
        >
          Construindo um<br />posicionamento inevitável
        </h1>
...
function Dores() {
  ...
  const dores = [
    ...
    },
    {
      ...
      descricao: 'Sua empresa existe, mas não é lembrada. Sem identidade forte, você vive na invisibilidade enquanto concorrentes menores se destacam.',
    },
    {
...
function TheOneFoundation() {
  ...
  return (
    <section ref={sectionRef} className="relative overflow-hidden bg-[#212121] px-6 md:px-12 lg:px-16 pt-16 md:pt-20 pb-24" style={{ minHeight: '100svh' }}>
      <div
        aria-hidden="true"
        style={{
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-10 lg:gap-16 items-start">
          <div>
            ...
            <div className="tf-item">
              <p className="font-halyard font-semibold text-[#FE6942] text-[12px] tracking-[0.22em] uppercase mb-5">O que você conquista</p>
              <ul className="grid grid-cols-2 gap-x-8 gap-y-3">
                {bullets.map((b, i) => (
                  <li key={i} className="flex items-center gap-3">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#FE6942] shrink-0" />
                    <span className="font-halyard font-light text-white/70 text-[18px] md:text-[19px] leading-[1.4]">{b}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div className="max-w-[1100px] mx-auto">
        ...
        {SERVICES.some(s => selected[s.id]) && (
          <div className="flex flex-col gap-5 mb-10 md:mb-14">
            ...
            {selected.mybranding && <CardMyBranding />}
...
function SobreTheOne({ scrollerRef }) {
  ...
  return (
    <div>
      ...
      {scroller && <HeroSection disableNavEvents scroller={scroller} />}
...
function PropostaSlideshow() {
  ...
  return (
    <div
      ...
      style={{ height: '100svh', background: ([0, 2, 3, 4, 5, 6, 7, 14].includes(current)) ? '#0a0a0a' : '#fff' }}
    >
      ...
      <div
        ...
        className={`absolute inset-0 overflow-y-auto ${([0, 2, 3, 4, 5, 6, 7, 14].includes(current)) ? 'pb-0' : 'pb-20 md:pb-16'} ${animDir === 'next' ? 'slide-from-right' : 'slide-from-left'}`}
      >
        {current === 0  && <Capa />}
        {current === 1  && <Dores />}
        {current === 2  && <SobreTheOne scrollerRef={slideScrollRef} />}
        {current === 3  && <SobreJean />}
        {current === 4  && <CaseSlide slug="zenic" />}
        {current === 5  && <CaseSlide slug="thunders" />}
        {current === 6  && <CaseSlide slug="camilla-toscano" />}
        {current === 7  && <TheOneFoundation />}
        {current === 8  && <CasaDaMarca />}
        {current === 9  && <EstrategiaDeMarca />}
        {current === 10 && <Naming />}
        {current === 11 && <IdentidadeVisual />}
        {current === 12 && <IdentidadeVisualCompleta />}
        {current === 13 && <MyBranding />}
        {current === 14 && <SiteBrandExperience />}
        {current === 15 && <TheOneAgent />}
        {current === 16 && <Cronograma />}
        {current === 17 && <Calculadora />}
      ...
      <div
        ...
        style={{
          ...
          background: `linear-gradient(to top, ${([0, 2, 3, 4, 5, 6, 7, 15].includes(current)) ? 'rgba(10,10,10,0.95)' : 'rgba(255,255,255,0.95)'} 0%, transparent 100%)`,
        }}
        aria-hidden="true"
      />
      ...
      <div className="absolute bottom-2 md:bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-3 z-20">
        <button
          ...
          style={{
            border: `1px solid ${([0, 2, 3, 4, 5, 6, 7, 14].includes(current)) ? 'rgba(255,255,255,.2)' : 'rgba(0,0,0,.15)'}`,
            background: ([0, 2, 3, 4, 5, 6, 7, 14].includes(current)) ? 'rgba(255,255,255,.1)' : 'rgba(255,255,255,.9)',
            backdropFilter: 'blur(8px)',
          }}
        >
          <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
            <path d="M10 12L6 8l4-4" stroke={([0, 2, 3, 4, 5, 6, 7, 14].includes(current)) ? 'white' : '#181412'} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
            ...
            <div
              ...
              style={{
                ...
                backgroundColor: i === current
                  ? '#FE6942'
                  : (([0, 2, 3, 4, 5, 6, 7, 14].includes(current)) ? 'rgba(255,255,255,.3)' : 'rgba(0,0,0,0.14)'),
              }}
            />
          ))}
        </div>
...
// ── PÁGINA ────────────────────────────────────────────────────────────────────
export default function PropostaPadraoP() {
  useEffect(() => {
