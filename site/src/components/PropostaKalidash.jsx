...
const SERVICES = [
  ...
  { id: 'entrevistas', label: 'Entrevistas com Clientes', price: 2000, prazo: '1 week' },
...
// 0: Capa | 1: Contexto A | 2: Contexto B | 3: Dores | 4: SobreTheOne | 5: Jean | 6: Zenic | 7: Thunders | 8: Camilla | 9: TheOne Foundation | 10: Casa da Marca | 11: Estratégia | 12: Naming | 13: Identidade Essencial | 14: Identidade Completa | 15: myBranding | 16: SiteBrandExperience | 17: TheOne Agent | 18: Cronograma | 19: Calculadora | 20: Consultoria
const SLIDE_TOTAL = 21;

const DARK_SLIDES = [0, 5, 6, 7, 8, 9, 17];
...
const PROPOSTA_DATA = {
  ...
  contextoA: {
    pontos: [
      'Mercado saturado com dezenas de consultorias de IA e automação; necessidade de diferenciação',
      'Falta de clareza sobre o rumo da comunicação e posicionamento, precisando começar a criar audiência do zero',
      'Ausência de uma narrativa de marca e de uma visão de futuro clara para o posicionamento',
      'Canais de comunicação e vendas atuais não estão bem estruturados',
      'O tráfego pago está ficando cada vez mais caro para atrair novos leads',
      'Apesar da alta satisfação com as soluções entregues, a percepção da marca ainda não acompanha esse nível'
    ],
  },
  contextoB: {
    pontos: [
      'Atingir mais público e atrair clientes de maior qualidade, expandindo o alcance',
      'Construir uma marca forte para se destacar no "mar vermelho" da IA e automação',
      'Fundadores posicionados como influencers e referências no mercado, fortalecendo a marca pessoal',
      'Escalar a venda de treinamentos in-company e fechar mais negócios B2B com indústrias',
      'Faturar R$ 2 milhões neste ano, utilizando a marca como pilar essencial',
      'Projeto ágil e focado, gerando impacto real no curto e médio prazo'
    ],
  },
};
  ...
  return (
    ...
    >
      ...
      >
        ...
        {current === 3  && <Dores />}
        {current === 4  && <SobreTheOne scrollerRef={slideScrollRef} />}
        {current === 5  && <SobreJean />}
        {current === 6  && <CaseSlide slug="zenic" />}
        {current === 7  && <CaseSlide slug="thunders" />}
        {current === 8  && <CaseSlide slug="camilla-toscano" />}
        {current === 9  && <TheOneFoundation />}
        {current === 10 && <CasaDaMarca />}
        {current === 11 && <EstrategiaDeMarca />}
        {current === 12 && <Naming />}
        {current === 13 && <IdentidadeVisual />}
        {current === 14 && <IdentidadeVisualCompleta />}
        {current === 15 && <MyBranding />}
        {current === 16 && <SiteBrandExperience />}
        {current === 17 && <TheOneAgent />}
        {current === 18 && <Cronograma />}
        {current === 19 && <Calculadora />}
      ...
      <div
        ...
        style={{
          ...
          background: `linear-gradient(to top, ${current === 4 ? 'rgba(128,128,128,0.15)' : (DARK_SLIDES.includes(current) ? 'rgba(10,10,10,0.95)' : 'rgba(255,255,255,0.95)')} 0%, transparent 100%)`,
        }}
        aria-hidden="true"
      />
...
// ── PÁGINA ────────────────────────────────────────────────────────────────────
export default function PropostaKalidash() {
  useEffect(() => {
